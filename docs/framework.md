...
- **mkdocs.yaml**: Per-repo config using `INHERIT: mkdocs-base.yaml` to inherit the framework's base theme, extensions, and plugins. Only repo-specific fields are defined here — `training_name`, `site_name`, `repo_name`, `repo_url`, `nav`, and `extra.rum_snippet`. A consuming repo defines no `theme:` and no `extra_css:` of its own; everything visual comes from the base.
- **`training_name`**: the training's display name, read by Orbital — **not an MkDocs key**. ENH-009 split a training's *identity* from its *name*: the identity is `<org>/<repo>` and is never shown, the name is only ever displayed. Orbital resolves the name through three rungs, in order: `training_name`, then `site_name` with the `Dynatrace Enablement/Observability Lab: ` prefix stripped, then the repo name. Declaring rung 1 lets a repo say what the training is *called* without that also having to be the docs site's page title. Two things about it are load-bearing. It must sit at **column 0** — Orbital anchors its match on line start, so moving it under `extra:` to tidy it away would parse fine as YAML and be invisible to Orbital. And MkDocs does not know the key, so **every build logs `Unrecognised configuration name: training_name`**; that warning is expected. Consuming repos build with plain `mkdocs build` and stay green, but this repository's own `validate-docs.yaml` uses `mkdocs build --strict`, where the same warning is fatal — so do not add `training_name` to this repository's `mkdocs.yaml` or to `mkdocs-base.yaml`.
- **mkdocs-base.yaml**: Framework-owned base configuration — Material theme, a dark-first custom Dynatrace palette, the markdown extensions, and the two asset references `extra_css: stylesheets/extra.css` and `extra_javascript: javascripts/mermaid.min.js`. Those assets live in this repository and are **fetched at build time** at the repo's pinned `FRAMEWORK_VERSION`, which is why none of the three files is committed in a consuming repo. A repo must fetch every asset the base references: MkDocs does not warn about a missing `extra_css` or `extra_javascript` target — not even under `--strict` — it just emits a tag pointing at a 404. `.github/scripts/check_referenced_assets.py` is the check that does catch it.
- **.github/workflows/deploy-ghpages.yaml**: GitHub Actions workflow that builds the site and publishes it to GitHub Pages. The trigger differs between this repository and the repos that consume it. **Here**, publication happens on push to `main` only — that is, after a pull request is merged. **In consuming repos**, it still triggers on push to `main` *and* `docs/*`, so a push to a `docs/*` branch publishes the public site before review.
- **.github/workflows/validate-docs.yaml**: builds the site with `mkdocs build --strict` and checks the referenced assets, **without publishing** — it holds no `pages: write` permission and no environment. It runs on pull requests to `main` and on pushes to `docs/**` and `rfe/**`. This workflow exists in this repository only; consuming repos have no build-without-publish check.
